...
export default function Article() {
  ...
  useEffect(() => {
    if (router.isReady) {
      ...
      const getData = Array.isArray(article) && article.find((value) => value.id === parseInt(id));
      if (getData) {
        setArticles(getData);
      }
    }
  }, [router.query.id, router.isReady, article]);
          ...
          <div className="w-[744px]">
            <div className="isiArticle flex gap-[28px] pt-[16px]">
              <div className="date flex items-center font-Poppins gap-[5px]">
                ...
                <p className="text-[16px]">{article?.date}</p>
              </div>
              <div className="admin items-center font-Poppins flex gap-[5px]">
                ...
                <p className="text-[16px]">{article?.name}</p>
              </div>
            </div>
            <h1 className="text-[44px] font-bold mb-[27px] text-left text-[#27005D]">{article?.title}</h1>
            <p className="text-[20px] font-normal my-[27px] text-slate-500 ">{article?.content}</p>

            <p className="text-[20px] font-normal my-[27px] text-slate-500 ">{article?.content}</p>

            <img src={article?.url} alt="" className="w-[744px] h-[558px] rounded-[30px] object-cover my-[27px]" />
            <p className="text-[20px] font-normal my-[27px] text-slate-500 ">{article?.content}</p>
          </div>
        </div>
      </div>
