...
const Service = () => {
  ...
  const getServicesById = async () => {
    const response = await axios.get(`http://localhost:3000/api/pengajuan/${editId}`);
    setServiceById(response.data.data);
  };

  const getServices = async () => {
    const response = await axios.get(`http://localhost:3000/api/pengajuan/user/${id}`);
    setService(response.data.data);
  };

  const deleteUsers = async (id) => {
    try {
      await axios.delete(`http://localhost:3000/api/pengajuan/delete/${id}`);
      setShowModal(false);
      handleRouter();
      getServices();
    } catch (error) {}
  ...
  const updateProses = async (id) => {
    ...
    try {
      await axios.patch(`http://localhost:3000/api/pengajuan/update/${id}`, formData, {
        headers: {
